...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('promotion')
    .setDescription('Promouvoir un employé (Direction uniquement)')
  ...
  async execute(interaction) {
    const canPromote = await PermissionService.canPromote(interaction.member);
    if (!canPromote) {
      throw new AppError('promotion: accès refusé', { userMessage: '❌ Seule la Direction peut promouvoir un employé.' });
    }
    ...
    await interaction.reply({
      embeds: [successEmbed('📈 Promotion effectuée', `<@${targetUser.id}> est désormais **${GRADE_LABELS[newGrade]}**.`)],
    });
  },
};
